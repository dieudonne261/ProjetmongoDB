      ...
      <div class="d-flex gap-5 justify-content-center py-2">
        ...
        <div class="modal fade show" id="editer" aria-modal="true" role="dialog" style="display: block;">
          <div class="modal-dialog modal-dialog-centered  modal-dialog-scrollable">
            <div class="modal-content bg-dark">
              <div class="modal-body">
                <p class="display-5 mx-5">Rechercher Livres</p>
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    if (!empty($_POST['code_barre'])) {
                        ...
                        $sql = "SELECT * FROM livres ".$_POST['optradio']." :code_barre";
                        $stmt = $db->prepare($sql);
                        $stmt->bindParam(':code_barre', $code_barre);
                        $stmt->execute();
                        if ($stmt->rowCount() > 0) {
                          echo '<table class="table table-dark table-borderless  justify-content-center">
                          <thead>
                          <tr class="h5">
                          ...
                          <tbody>';
                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                              echo "<tr>";
                              echo "<td class='h6'>" . $row['titre'] . "</td>";
                              echo "<td class='h6'>" . $row['auteur'] . "</td>";
                              echo "<td class='h6'>" . $row['date_ed'] . "</td>";
                            ...
                            echo "<p class='display-6'>Aucun résultat trouvé</p>";
                        }
                    } else {
                        echo "<p class='display-6'>Erreur d'entree</p>";
                    }
